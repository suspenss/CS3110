...
module Promise : PROMISE = struct
  ...
  let update (p : 'a state ref) (s : 'a state) : unit =
    match !p with
    | Pending -> p := s
  ...
  (* *)
  let make (() : unit) : 'a promise * 'a resolver =
    let p = ref Pending in
    p, p
  ;;

  let return (x : 'a) : 'a promise = ref (Resolved x)
  let state (p : 'a promise) : 'a state = !p
  let resolve (r : 'a resolver) (x : 'a) : unit = update r (Resolved x)
  let reject (r : 'a resolver) (x : exn) : unit = update r (Rejected x)
end
...
module Lwt = struct
  ...
  let update (p : 'a t) (s : 'a state) : unit =
    match !p with
    | Sleep -> p := s
    | _ -> invalid_arg "State has been updated"
  ;;

  let state (p : 'a t) : 'a state = !p
  let wakeup (r : 'a u) (x : 'a) : unit = update r (Return x)
  let wakeup_exn (r : 'a u) (x : exn) : unit = update r (Fail x)
  let return (x : 'a) : 'a t = ref (Return x)
end

(** A signature for Lwt-style promises, with better names *)
module type lwtstyle = sig
  type 'a state =
    | Pending
    | Resolved of 'a
    | Rejected of exn
...
module LwtPromise : lwtstyle = struct
  type 'a state =
    | Pending
    | Resolved of 'a
    | Rejected of exn
  ...
  let write_once p s =
    if p.state = Pending then
      ...
      invalid_arg "cannot write twice"
  ;;
